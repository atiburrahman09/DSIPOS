...
namespace lmxIpos.UI.DamageRecordToWH
{
    public partial class DamageRecord : System.Web.UI.Page
    {
        ...
        protected void GetAvailableProductListByWarehouse()
        {
            ...
            try
            {
                DataTable dt = product.GetAvailableProductListByWarehouse(warehouseDropDownList.SelectedValue);
        ...
        protected void saveButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                salesOrder.SalesCenterId = warehouseDropDownList.SelectedValue;
                salesOrder.TotalAmount = totalAmountTextBox.Text.Trim();

                if (dtPrdList.Rows.Count == selectedProductListGridView.Rows.Count)
                {
                    string damageRecordId = salesOrder.SaveWarehouseDamageProduct(dtPrdList);
                    ...
                    MyAlertBox("var callbackOk = function () { MyOverlayStart(); window.location = \"/UI/DamageRecordToWH/DamageRecordList.aspx\"; }; SuccessAlert(\"" + "Process Succeed" + "\", \"" + message + "\", callbackOk);");
                }
            }
            catch (Exception ex)
            {
