...
namespace Lumex.Project.DAL
{
    public class ProductPriceDAL
    {
        ...
        public void UpdateProductPriceList(List<ProductPriceBLL> productPrices, LumexDBPlayer db)
        {
            try
            {
                for (int i = 0; i < productPrices.Count; i++)
                {
                    ...
                    db.AddParameters("@BuyRatePerUnit", productPrices[i].NewBuyRate);
        ...
        internal DataTable GetProductPriceListByUser(LumexDBPlayer db)
        {
            try
            {
                db.AddParameters("@UserId", LumexSessionManager.Get("ActiveUserId").ToString());
                DataTable dt = db.ExecuteDataTable("[GET_ACTIVE_PRODUCT_PRICE_LIST_BY_ACTIVE_USER]", true);
                return dt;
            }
            catch (Exception)
            {
