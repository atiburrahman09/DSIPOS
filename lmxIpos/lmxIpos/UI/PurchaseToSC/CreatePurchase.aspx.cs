...
namespace lmxIpos.UI.PurchaseToSC
{
    public partial class CreatePurchase : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            vendorOrderDateTextBox.Attributes.Add("autocomplete", "off");
            receivedDateTextBox.Attributes.Add("autocomplete", "off");
        ...
        protected void LoadSalesCenters()
        {
            ...
            try
            {
                ...
                salesCenterDropDownList_SelectedIndexChanged(this, new EventArgs());
        ...
        protected void addProductButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string productBarcodeIdName = id[0]; //productTextBox.Text.Trim();

                if (string.IsNullOrEmpty(productBarcodeIdName))
                {
                    msgbox.Visible = true; msgTitleLabel.Text = "Exception!!!"; msgDetailLabel.Text = "Write Product Barcode or ID or Name to search."; msgbox.Attributes.Add("class", "alert alert-warning");
                }
                else
                {
                    if (CheckAddedProduct(productBarcodeIdName))
                    {
                        msgbox.Visible = true; msgTitleLabel.Text = "Exception!!!"; msgDetailLabel.Text = "This product already added in the selected product list."; msgbox.Attributes.Add("class", "alert alert-warning");
                    }
                    else
                    {
        ...
        protected void saveButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < purchaseProductListGridView.Rows.Count; i++)
                {
                    ...
                    if (!int.TryParse(quantityTextBox.Text.Trim(), out num))
                    {
                ...
                if (dt.Rows.Count == purchaseProductListGridView.Rows.Count)
                {
                    ...
                    purchaseRecord.SalesCenterId = salesCenterDropDownList.SelectedValue;
                    ...
                    if (!string.IsNullOrEmpty(id))
                    {
                        ...
                        MyAlertBox("var callbackOk = function () { MyOverlayStart(); window.location = \"/UI/PurchaseToSC/PurchaseList.aspx\"; }; SuccessAlert(\"" + "Process Succeed" + "\", \"" + message + "\", callbackOk);");

                        // MyAlertBox("alert(\"Purchase Record Created Successfully with Purchase Record ID: " + id + " \"); window.location=\"/UI/PurchaseToSC/PurchaseList.aspx\"");
                    }
                    else
                    {
                        string message = "Purchase Record <span class='actionTopic'>Created</span> Is Faild </span>.";
                        MyAlertBox("var callbackOk = function () { MyOverlayStart(); window.location = \"/UI/PurchaseToSC/PurchaseList.aspx\"; }; ErrorAlert(\"" + "Process Succeed" + "\", \"" + message + "\", callbackOk);");
        ...
        protected void LoadCashorBankAccountHead()
        {
            try
            {
                try
