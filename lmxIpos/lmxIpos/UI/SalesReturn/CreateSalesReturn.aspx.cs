...
namespace lmxIpos.UI.SalesReturn
{
    public partial class CreateSalesReturn : System.Web.UI.Page
    {
        ...
        protected void getSalesDetailsButton_Click(object sender, EventArgs e)
        {
            ...
            UncollectedAmountLabel.Text = "";
            totalReceivableLabel.Text = "";
            CreditStatusLabel.Text = "";
        ...
        protected void addProductButton_Click(object sender, EventArgs e)
        {
            if (salesRecordProductListGridView.Rows.Count > 0)
            {
                ...
                decimal PerUnitServiceCharge = decimal.Parse(ServiceChargeLabel.Text) / decimal.Parse(totalAmountLabel.Text);
                decimal totalServiceCharge = amount * PerUnitServiceCharge;
                decimal tAmt = amount - amount * perUnitDiscount;
                //decimal totalVAT = tAmt * perUnitDiscount;//(tAmt * decimal.Parse(vatPercentageLabel.Text)) / 100;
               
               // tAmt = tAmt - totalVAT;//- totalServiceCharge;

                totalReturnVATAmountTextBox.Text = totalServiceCharge.ToString();
        ...
        protected void SaveSalesReturn(string isDuesAdjust)
        {
            ...
            try
            {
                ...
                if (isDuesAdjust == "Y")
                {
                    ...
                    string message =
                        "Sales <span class='actionTopic'>Return</span> Successfully with Adjust Dues of Return ID: <span class='actionTopic'>" + dt.Rows[0]["SalesReturnId"] + " And Money Back Amount:" + dt.Rows[0]["ReturnAmount"] + "</span>.";
                    MyAlertBox(
                ...
                {
                    string message =
                       "Sales <span class='actionTopic'>Return</span> Successfully with Money Back Only of Return ID: <span class='actionTopic'>" + dt.Rows[0]["SalesReturnId"] + " And Money Back Amount:" + dt.Rows[0]["ReturnAmount"] + "</span>.";
                    MyAlertBox(
                                   "var callbackOk = function () { MyOverlayStart(); window.location = \"/UI/SalesReturn/SalesReturnList.aspx\"; }; SuccessAlert(\"" +
                                   "Process Succeed" + "\", \"" + message + "\", callbackOk);");
